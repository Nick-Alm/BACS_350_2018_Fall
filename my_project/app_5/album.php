<?php
    ...
    $page = 'index.php';
    // Add a new record
    function add_album($db, $name, $artist, $artwork, $purchase_url, $description, $review) {
        // Show if insert is successful or not
        try {
            // Add database row
            $query = "INSERT INTO albums (artist, name, artwork, purchase_url, description, review) VALUES (:artist, :name, :artwork, :purchase_url, :description, :review);";
    ...
    function update_album ($db) {
        ...
        // Modify database row
        $query = "UPDATE albums SET artist = :artist, name = :name,  purchase_url = :purchase_url, artwork = :artwork, description = :description, review = :review WHERE id = :id";
        $statement = $db->prepare($query);
        $statement->bindValue(':id', $id);
        $statement->bindValue(':artist', $artist);
        $statement->bindValue(':name', $name);
